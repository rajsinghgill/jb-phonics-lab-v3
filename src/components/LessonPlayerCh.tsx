...
// --- CONFIGURATION ---
const VIDEO_URL =
  "https://www.dropbox.com/scl/fi/y6fi4offda102o54z4vvx/letter-of-ch.mp4?rlkey=jj9mxeq78gz0tqrh2dqzh10xw&st=3l6w6sqk&raw=1";
const GAME_URL =
  "https://rajsinghgill.github.io/jb-phonics-lab/Hungry%20Snake/hungry-snake.html";
const STOPS = [55, 151, 9999];
...
const LESSON_SUMMARY = {
  sound: "ch",
  soundAudio: "ch-sound.mp3",
  words: [
    { text: "Chat", icon: "🗣️", audio: "chat-sound.mp3" },
    { text: "Chip", icon: "🍟", audio: "chip-sound.mp3" },
    { text: "Chop", icon: "🪓", audio: "chop-sound.mp3" },
    { text: "Chug", icon: "🚂", audio: "chug-sound.mp3" },
  ] as VocabWord[],
  sentences: [
    {
      text: "I chat with my friend.",
      audio: "i-chat-with-my-friend-sound.mp3",
    },
    { text: "The train goes chug.", audio: "the-train-goes-chug-sound.mp3" },
    { text: "I chop the big tree.", audio: "i-chop-the-big-log-sound.mp3" },
  ],
};

// --- QUIZ DATA ---
const Q1_ITEMS: QuizItem[] = [
  { id: "chat", label: "Chat", correct: true, icon: "🗣️" },
  { id: "cat", label: "Cat", correct: false, icon: "🐱" },
  { id: "chop", label: "Chop", correct: true, icon: "🪓" },
  { id: "ball", label: "Ball", correct: false, icon: "⚽" },
  { id: "chug", label: "Chug", correct: true, icon: "🚂" },
  { id: "dog", label: "Dog", correct: false, icon: "🐶" },
];

const Q2_WORDS: CookingItem[] = [
  { id: "chat", rime: "at", icon: "🗣️" },
  { id: "chop", rime: "op", icon: "🪓" },
  { id: "chug", rime: "ug", icon: "🚂" },
];

const Q3_DATA = [
  {
    target: ["I", "chat", "with", "my", "friend"],
    scramble: ["chat", "I", "friend", "with", "my"],
  },
  {
    target: ["The", "train", "goes", "chug"],
    scramble: ["goes", "chug", "The", "train"],
  },
  {
    target: ["I", "chop", "the", "big", "log"],
    scramble: ["big", "chop", "I", "log", "the"],
  },
];

// --- HELPER COMPONENT: Highlights 'ch' or 'Ch' ---
const HighlightCh: React.FC<{ text: string }> = ({ text }) => {
  // Split by 'ch' (case insensitive) keeping the delimiter
  const parts = text.split(/(ch)/gi);
  return (
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === "ch" ? (
...
const LessonPlayerCh: React.FC<LessonPlayerProps> = ({
  onBack,
  onLogout,
  userName = "Student",
}) => {
  ...
  const getStartTime = () => {
    if (stage === 0) return 0;
    if (stage === 2) return 58;
    if (stage === 4) return 152;
    return 0;
  };
  ...
  const [q1WrongShake, setQ1WrongShake] = useState<string | null>(null);
  ...
  // --- QUIZ HANDLERS ---
  const handleQ1Click = (item: QuizItem) => {
    if (item.correct) {
      if (!q1Selected.includes(item.id)) {
        playAudio(null, LESSON_SUMMARY.soundAudio);
        setQ1Selected((prev) => [...prev, item.id]);
      }
    } else {
      setQ1WrongShake(item.id);
      setTimeout(() => setQ1WrongShake(null), 600);
    }
  };

  useEffect(() => {
    const correctIds = Q1_ITEMS.filter((i) => i.correct).map((i) => i.id);
    const isDone =
      correctIds.every((id) => q1Selected.includes(id)) &&
      q1Selected.length === correctIds.length;
    if (isDone && stage === 1) setIsCompleted(true);
  }, [q1Selected, stage]);
  ...
  const currentSentenceData = Q3_DATA[q3Index];
  const handleWordClick = (word: string) => {
    const nextIndex = q3BuiltSentence.length;
    if (word === currentSentenceData.target[nextIndex]) {
  ...
  useEffect(() => {
    if (
      q3BuiltSentence.length === currentSentenceData.target.length &&
  ...
  }, [q3BuiltSentence, currentSentenceData.target.length, q3Index]);
        ...
        return (
          <div className="flex flex-col items-center gap-6 w-full">
            <h2 className="text-3xl md:text-4xl font-black text-gray-800 text-center">
              Load the <span className="text-blue-600">Train!</span> <br />
              Click the words that start with{" "}
              <span className="text-red-500 font-black">/ch/</span>
            </h2>
            <div className="bg-blue-50 px-6 py-2 rounded-full border-2 border-blue-200 flex items-center gap-2 mb-2 animate-bounce">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              {Q1_ITEMS.map((item) => (
                <button
                  key={item.id}
                  onClick={() => handleQ1Click(item)}
                  disabled={q1Selected.includes(item.id)}
                  className={`relative p-6 rounded-3xl border-b-8 transition-all flex flex-col items-center gap-2 ${
                    q1Selected.includes(item.id)
                      ? "bg-green-100 border-green-500 scale-95 opacity-50 cursor-default"
                      : q1WrongShake === item.id
                      ? "bg-red-200 border-red-500 animate-shake"
                      : "bg-white border-blue-200 hover:translate-y-[-4px] shadow-lg"
                  }`}
                >
        ...
        return (
          <div className="flex flex-col items-center gap-10 w-full">
            ...
            <div className="w-full flex justify-center py-4">
              <div
                draggable
                onDragStart={() => setDraggedItem("ch")}
                className="w-24 h-24 bg-red-500 text-white rounded-2xl shadow-xl border-b-8 border-red-700 flex items-center justify-center text-5xl font-black cursor-grab active:cursor-grabbing hover:scale-110 transition-transform z-20"
              >
                ch
              </div>
            </div>
            <div className="flex flex-wrap justify-center gap-6 w-full">
              {Q2_WORDS.map((word) => (
                ...
                >
                  ...
                  <div className="flex items-center text-5xl font-black text-gray-700">
                    <div
                      className={`w-20 h-16 mr-1 rounded-lg flex items-center justify-center transition-all ${
                        q2Filled[word.id]
          ...
          <div className="flex flex-col items-center gap-8 w-full">
            ...
            {q3ShowSuccess && (
              <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
                <div className="bg-green-500 text-white px-8 py-4 rounded-3xl shadow-2xl animate-bounce text-3xl font-black">
                  Correct! 🎉
                </div>
              </div>
            )}
            ...
            >
              {q3BuiltSentence.length === 0 && (
                <span className="text-gray-300 font-bold text-2xl italic">
                  Tap words below...
                </span>
              )}
              {q3BuiltSentence.map((word, i) => (
                <div
                  key={i}
                  className="px-6 py-3 bg-blue-100 text-blue-800 rounded-xl font-black text-2xl border-2 border-blue-300 animate-in fade-in zoom-in"
                >
                  {word}
                </div>
              ))}
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-4">
              {currentSentenceData.scramble.map((word, index) => {
                const totalInBuilt = q3BuiltSentence.filter(
                  (w) => w === word
                ).length;
                const previousInstances = currentSentenceData.scramble
                  .slice(0, index)
                ...
                return (
                  <button
                    ...
                    className="w-auto px-8 py-4 bg-orange-500 text-white rounded-2xl text-2xl font-black border-b-4 border-orange-700 hover:scale-105 active:scale-95 transition-all shadow-lg"
                  >
                    {word}
                  </button>
                );
      ...
      case 7:
        return (
          <div className="text-center space-y-8 py-10">
            ...
            <h1 className="font-header text-6xl font-black text-blue-600 drop-shadow-md">
              AWESOME!
            </h1>
            <p className="text-3xl font-bold text-gray-700 italic">
              You mastered the sound{" "}
              <span className="text-red-500 text-5xl font-black">ch</span>!
            </p>
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-8 border-4 border-white shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="font-body text-slate-600 text-lg">
                ...
                <strong className="text-red-600">/ch/</strong> sound. It's the
                sound in 'Chair' and 'Chat'!
              </p>
            </div>
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-4">
            ...
            <button
              onClick={() => {
                setStage(0);
                setQ1Selected([]);
                setQ1WrongShake(null);
                setQ2Filled({ chat: false, chop: false, chug: false });
                setQ3Index(0);
                setQ3ShowSuccess(false);
                setQ3BuiltSentence([]);
                setIsCompleted(false);
