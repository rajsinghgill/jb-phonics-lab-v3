...
// --- CONFIGURATION ---
const VIDEO_URL =
  "https://www.dropbox.com/scl/fi/yl2trrc3nw1ixuazgszpd/Letter-of-p_ver-2.mp4?rlkey=944vc2baiexjdvehq8r8bj0r6&st=rzsszl74&raw=1";
const GAME_URL =
  "https://rajsinghgill.github.io/jb-phonics-lab/Chefs%20Challenge/chefs-challenge.html";

// TIMINGS (Seconds)
// Part 1: Intro -> Activity 1 (Ends at 50s)
// Part 2: Middle -> Activity 2 (Ends at 117s - Extended by 1s)
const STOPS = [50, 117, 9999];
...
const LESSON_SUMMARY = {
  sound: "p",
  soundAudio: "p-sound.mp3",
  words: [
    { text: "Pot", icon: "🍲", audio: "pot-sound.mp3" },
    { text: "Pet", icon: "🐹", audio: "pet-sound.mp3" },
    { text: "Play", icon: "🛝", audio: "play-sound.mp3" },
    { text: "Park", icon: "🌳", audio: "park-sound.mp3" },
  ] as VocabWord[],
  sentences: [
    {
      text: "Children play in the park.",
      audio: "children-play-in-the-park-sound.mp3",
    },
    { text: "Pem's pet is a cat.", audio: "pems-pet-is-a-cat-sound.mp3" },
    {
      text: "We play with the puppy.",
      audio: "we-play-with-the-puppy-sound.mp3",
    },
  ],
};
...
const Q1_ITEMS: QuizItem[] = [
  { id: "pot", label: "Pot", isP: true, icon: "🍲" },
  { id: "dog", label: "Dog", isP: false, icon: "🐶" },
  { id: "pet", label: "Pet", isP: true, icon: "🐹" },
  { id: "play", label: "Play", isP: true, icon: "🛝" },
  { id: "sun", label: "Sun", isP: false, icon: "☀️" },
  { id: "park", label: "Park", isP: true, icon: "🌳" },
];

const Q2_WORDS: CookingItem[] = [
  { id: "pot", rime: "ot", icon: "🍲" },
  { id: "pet", rime: "et", icon: "🐈" },
  { id: "park", rime: "ark", icon: "🌳" },
];

const Q3_DATA = [
  {
    target: ["Children", "play", "in", "the", "park"],
    scramble: ["park", "play", "Children", "the", "in"],
  },
  {
    target: ["Pem's", "pet", "is", "a", "cat"],
    scramble: ["cat", "Pem's", "is", "a", "pet"],
  },
  {
    target: ["We", "play", "with", "the", "puppy"],
    scramble: ["with", "We", "puppy", "the", "play"],
  },
];

// --- HELPER COMPONENT: Highlights 'p' or 'P' at start of words ---
const HighlightP: React.FC<{ text: string }> = ({ text }) => {
  const parts = text.split(" ");
  return (
    <span>
      {parts.map((word, i) => {
        const startsWithP = word.toLowerCase().startsWith("p");
        return (
          <span key={i}>
            {startsWithP ? (
              <>
                <span className="text-pink-500">{word.charAt(0)}</span>
                {word.slice(1)}
              </>
            ) : (
              word
...
const LessonPlayerP: React.FC<LessonPlayerProps> = ({
  onBack,
  onLogout,
  userName = "Student",
}) => {
  ...
  const getStartTime = () => {
    if (stage === 0) return 0;
    if (stage === 2) return 51; // Start of Part 2
    if (stage === 4) return 117; // Start of Part 3 (Synced with new stop time)
    return 0;
  };
  ...
  useEffect(() => {
    const correctIds = Q1_ITEMS.filter((i) => i.isP).map((i) => i.id);
        ...
        return (
          <div className="flex flex-col items-center gap-8 w-full">
            <h2 className="font-header text-3xl md:text-4xl font-bold text-gray-800 text-center">
              Tap the bubbles with the{" "}
              <span className="text-pink-500">/p/</span> sound!
            </h2>
            <div className="flex flex-wrap justify-center gap-6 max-w-4xl">
              ...
              {Q1_ITEMS.filter((i) => i.isP).every((i) =>
      ...
      case 3:
        return (
          <div className="flex flex-col items-center gap-10 w-full">
            <h2 className="font-header text-3xl md:text-4xl font-bold text-gray-800 text-center">
              Drag{" "}
              <span className="text-pink-500 px-2 bg-pink-100 rounded-lg mx-1">
                p
              </span>{" "}
            ...
            <div className="w-full flex justify-center py-4">
              <div
                ...
                className="w-24 h-24 bg-pink-500 text-white rounded-full shadow-xl border-b-8 border-pink-700 flex items-center justify-center text-5xl font-header font-bold cursor-grab active:cursor-grabbing hover:scale-110 transition-transform z-20"
              >
                p
              </div>
            </div>
          ...
          <div className="flex flex-col items-center gap-8 w-full">
            ...
            >
              ...
              {q3BuiltSentence.map((word, i) => (
                <div
                  key={i}
                  className="px-6 py-3 bg-pink-100 text-pink-800 rounded-xl font-header font-bold text-2xl border-2 border-pink-300 animate-in fade-in zoom-in"
                >
                  {word}
                </div>
              ))}
        ...
        return (
          <div className="text-center space-y-8 py-10">
            ...
            <p className="font-header text-3xl font-bold text-gray-700 italic">
              ...
              <span className="text-pink-500 text-5xl">p</span>!
            </p>
      ...
      <main className="flex-1 flex flex-col items-center p-4 md:p-10 z-0 gap-10">
        ...
        <div className="w-full max-w-5xl flex flex-col gap-8 animate-fade-in-up pb-10">
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-8 border-4 border-white shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-pink-100 rounded-full -translate-y-1/2 translate-x-1/2 opacity-50 group-hover:scale-110 transition-transform"></div>

            <div
              className="bg-pink-100 p-6 rounded-3xl border-4 border-pink-200 cursor-pointer hover:scale-110 transition-transform"
              onClick={(e) => playAudio(e, LESSON_SUMMARY.soundAudio)}
            >
              <Volume2 size={48} className="text-pink-500" />
            </div>

            <div className="flex-1 text-center md:text-left">
              <h3 className="font-header text-3xl font-bold text-pink-900 mb-2">
                Target Sound
              </h3>
              <p className="font-body text-slate-600 text-lg">
                In this lesson, we mastered the{" "}
                <strong className="text-pink-600">/p/</strong> sound. This is a
                "plosive" sound made by popping your lips together!
              </p>
            </div>

            <div
              className="text-7xl font-header font-bold text-white bg-pink-500 w-32 h-32 rounded-3xl flex items-center justify-center border-b-8 border-pink-700 shadow-2xl transform rotate-3 hover:rotate-0 transition-transform cursor-pointer"
              onClick={(e) => playAudio(e, LESSON_SUMMARY.soundAudio)}
            >
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-4">
            ...
            <button
              onClick={() => {
                setStage(0);
                setQ1Popped([]);
                setQ2Filled({ pot: false, pet: false, park: false });
                setQ3Index(0);
                setQ3BuiltSentence([]);
                setIsCompleted(false);
