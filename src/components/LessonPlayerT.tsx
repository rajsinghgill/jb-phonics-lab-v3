...
// --- CONFIGURATION ---
const VIDEO_URL =
  "https://www.dropbox.com/scl/fi/7duc995ez2gkgq633eisf/Letter-of-T.mp4?rlkey=5qytdyuidfhunz3mwzikl4yhk&st=hyd41l29&raw=1";
const GAME_URL =
  "https://rajsinghgill.github.io/jb-phonics-lab/t-rex%20game/trex-game.html";

// UPDATED TIMINGS (in seconds)
// Part 1 ends at 1:11 (71s)
// Part 2 ends at 2:55 (175s)
const STOPS = [71, 175, 9999];
...
const LESSON_SUMMARY = {
  sound: "t",
  soundAudio: "t-sound.mp3",
  words: [
    { text: "Table", icon: "🪑", audio: "table-sound.mp3" },
    { text: "Teacher", icon: "👩‍🏫", audio: "teacher-sound.mp3" },
    { text: "Tree", icon: "🌳", audio: "tree-sound.mp3" },
    { text: "Tiger", icon: "🐯", audio: "tiger-sound.mp3" },
  ] as VocabWord[],
  sentences: [
    {
      text: "Tim sits at the table.",
      audio: "tim-sits-at-the-table-sound.mp3",
    },
    {
      text: "A shark has sharp teeth.",
      audio: "a-shark-has-sharp-teeth-sound.mp3",
    },
    {
      text: "A tall tree has twigs.",
      audio: "a-tall-tree-has-twigs-sound.mp3",
    },
  ],
};
...
const Q1_ITEMS: QuizItem[] = [
  { id: "table", label: "Table", isTarget: true, icon: "🪑" },
  { id: "dog", label: "Dog", isTarget: false, icon: "🐶" },
  { id: "teacher", label: "Teacher", isTarget: true, icon: "👩‍🏫" },
  { id: "tree", label: "Tree", isTarget: true, icon: "🌳" },
  { id: "sun", label: "Sun", isTarget: false, icon: "☀️" },
  { id: "tiger", label: "Tiger", isTarget: true, icon: "🐯" },
];

const Q2_WORDS: CookingItem[] = [
  { id: "table", rime: "able", icon: "🪑" },
  { id: "tree", rime: "ree", icon: "🌳" },
  { id: "teacher", rime: "eacher", icon: "👩‍🏫" },
];

const Q3_DATA = [
  {
    target: ["Tim", "sits", "at", "the", "table"],
    scramble: ["table", "Tim", "the", "at", "sits"],
  },
  {
    target: ["A", "shark", "has", "sharp", "teeth"],
    scramble: ["teeth", "shark", "A", "sharp", "has"],
  },
  {
    target: ["A", "tall", "tree", "has", "twigs"],
    scramble: ["tree", "A", "twigs", "has", "tall"],
  },
];

// --- HELPER COMPONENT ---
const HighlightT: React.FC<{ text: string }> = ({ text }) => {
  const parts = text.split(" ");
  return (
    <span>
      {parts.map((word, i) => {
        const startsWithT = word.toLowerCase().startsWith("t");
        return (
          <span key={i}>
            {startsWithT ? (
              <>
                <span className="text-red-500">{word.charAt(0)}</span>
                {word.slice(1)}
              </>
            ) : (
              word
...
const LessonPlayerT: React.FC<LessonPlayerProps> = ({
  onBack,
  onLogout,
  userName = "Student",
}) => {
  ...
  const getStartTime = () => {
    if (stage === 0) return 0;
    // Part 2 starts at 1:12 = 72s
    if (stage === 2) return 72;
    // Part 3 starts at 2:58 = 178s
    if (stage === 4) return 178;
    return 0;
  };
        ...
        return (
          <div className="flex flex-col items-center gap-8 w-full">
            <h2 className="font-header text-3xl md:text-4xl font-bold text-gray-800 text-center">
              Tap the bubbles with the <span className="text-red-500">/t/</span>{" "}
              sound!
            </h2>
        ...
        return (
          <div className="text-center space-y-8 py-10">
            ...
            <p className="font-header text-3xl font-bold text-gray-700 italic">
              ...
              <span className="text-red-500 text-5xl">t</span>!
            </p>
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-8 border-4 border-white shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="font-body text-slate-600 text-lg">
                ...
                <strong className="text-red-600">/t/</strong> sound. This is a
                short sound, like in Table and Teacher!
              </p>
            </div>
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-4">
            ...
            <button
              onClick={() => {
                setStage(0);
                setQ1Popped([]);
                setQ2Filled({ table: false, tree: false, teacher: false });
                setQ3Index(0);
                setQ3BuiltSentence([]);
                setIsCompleted(false);
            ...
            <div className="flex gap-4 w-full justify-center">
              <button
                onClick={(e) =>
                  playAudio(
                    e,
                    "https://raw.githubusercontent.com/YOUR_USER/YOUR_REPO/main/sounds/t-sound.mp3"
                  )
                }
              ...
              >
                <Volume2 size={24} /> /t/ Sound
              </button>
              <button
                onClick={(e) =>
                  playAudio(
                    e,
                    `https://raw.githubusercontent.com/YOUR_USER/YOUR_REPO/main/sounds/${LESSON_SUMMARY.words[
                      currentFlashcard
                    ].text.toLowerCase()}.mp3`
                  )
                }
